...
#if defined(NDEBUG)
# error "Avana cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x94e2856abb6d4df4df7bd3a8c262822b9d549d1a104c4d2ddefc97a3f34e71f0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Avana: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Avana Signed Message:\n";
...
{
    // Avana: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Avana: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Avana: 3.5 days
static const int64 nTargetSpacing = 10 * 60; // Avana: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Avana: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Avana: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2ec3858419642f7c38b19c717bc86e1386527f5d6b4d065a05122b4b5b31a58c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = " 21/4/2020 - Panademic";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 2700000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04efd1362e113a472387d70befcbed556cb6d2b36ad3ff766332c16b907b418d830368605c140dd624a23fccc724027fb88c46714420067cdeaf04f3478d4cf2e3") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1587537316;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084596530;

        if (fTestNet)
        {
            block.nTime    = 1587537287;
            block.nNonce   = 387645763;
        }
...
unsigned char pchMessageStart[4] = { 0xfu, 0xc7, 0xb4, 0xdf }; // Avana: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AvanaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AvanaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AvanaMiner(CWallet *pwallet)
{
    printf("AvanaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Avana-miner");
    ...
    try { loop {
        ...
        printf("Running AvanaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AvanaMiner, pwallet));
}
